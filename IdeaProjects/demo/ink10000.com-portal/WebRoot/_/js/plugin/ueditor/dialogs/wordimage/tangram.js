...
baidu.each=baidu.array.forEach=baidu.array.each=function(source,iterator,thisObject){
	if("function"==typeof iterator){
		for(var i=0;i<source.length;i++){
			var returnValue=iterator.call(thisObject||source,source[i],i);
	...
	if(n.plugins&&n.mimeTypes.length){
		...
		for(var i=12;i>=2;i--){
			try{
				...
				if(c){
					return c.GetVariable("$version").replace(/WIN/g,"").replace(/,/g,".");
				}
			}catch(e){}
		}
	}
...
baidu.swf.createHTML=function(options){
	options=options||{};
	var version=baidu.swf.version,tmpOpt={},encodeHTML=baidu.string.encodeHTML;
	...
	if(version){
		...
		var needVersion=(options["ver"]||"6.0.0").split(".");
		for(var i=0;i<3;i++){
			var vUnit1=parseInt(version[i],10);
			var vUnit2=parseInt(needVersion[i],10);
			if(vUnit2<vUnit1){
				break;
	...
	for(var k in options){
		...
		if(params[k]&&(item||item===false||item===0)){
			str.push("<param value=\""+encodeHTML(item)+"\" name=\""+k+"\"/>");
		}
	}
...
baidu.swf.create=function(options,target){
	...
	baidu.dom.insertHTML(target||document.body,"beforeEnd",baidu.swf.createHTML(options)||options["errorMessage"]||"");
};
...
baidu.swf.getMovie=function(name){
	...
	return baidu.browser.ie==9?movie&&movie.length?(ret=baidu.array.remove(baidu.lang.toArray(movie),function(item){return item.tagName.toLowerCase()!="embed";})).length==1?ret[0]:ret:movie:movie||window[name];
};
baidu.flash._Base=(function(){
	...
	function _callFn(callQueue,target){
		...
		baidu.each(callQueue,function(item){
			item.callBack(target.call(item.fnName,item.params));
		});
	};
	function _createFunName(fun){
		if(baidu.lang.isFunction(fun)){
			var name=_createString();
	...
	return function(options,callBack){
		var me=this,autoRender=typeof options.autoRender!=="undefined"?options.autoRender:true,target=null,isReady=false,callQueue=new Array(),timeHandle=null,createOptions=options.createOptions||{};
		callBack=callBack||[];
		me.render=function(){
		...
		me.call=function(fnName,params,callBack){
			...
			if(isReady){
				...
				!timeHandle&&(timeHandle=window.setInterval(_check,200));
			}
		};
...
baidu.flash.imageUploader=baidu.flash.imageUploader||function(options){
	options=options||{};
	var me=this,_flash=new baidu.flash._Base(options,["selectFileCallback","exceedFileCallback","deleteFileCallback","startUploadCallback","uploadCompleteCallback","uploadErrorCallback","allCompleteCallback","changeFlashHeight"]);
	me.upload=function(){
		_flash.call("upload");
	};
	me.pause=function(){
...
baidu.flash.fileUploader=baidu.flash.fileUploader||function(options){
	...
	me.addFileType=function(type){
		_flash.call("addFileTypes",type instanceof Array?[type]:[[type]]);
	};
	me.setFileType=function(type){
		_flash.call("setFileTypes",type instanceof Array?[type]:[[type]]);
	};
...
baidu.sio.callByBrowser=function(url,opt_callback,opt_options){
	var scr=document.createElement("SCRIPT"),scriptLoaded=0,options=opt_options||{},charset=options["charset"],callback=opt_callback||function(){},timeOut=options["timeOut"]||0,timer=null;
...
baidu.sio.callByServer=function(url,callback,opt_options){
	var scr=document.createElement("SCRIPT"),prefix="bd__cbs__",callbackName=null,options=opt_options||{},charset=options["charset"],queryField=options["queryField"]||"callback",	timeOut=options["timeOut"]||0,timer=null,reg=new RegExp("(\\?|&)"+queryField+"=([^&]*)"),matches;
	...
	if(timeOut){
		timer=window.setTimeout(getCallBack(1),timeOut);
	}
			...
			switch(typeof value){
				...
				default:
					if(value===null){
						...
						var result=new Array("{"),encode=baidu.json.stringify,preComma=null;
