...
function addOkListener(){
	dialog.onok=function(){
		...
		var urlPrefix=editor.getOpt("imageUrlPrefix"),images=domUtils.getElementsByTagName(editor.document,"img");
...
function showLocalPath(id){
	...
	var path=images[0];
	path=path.substring(0,path.lastIndexOf((path.lastIndexOf("/")||0)>(path.lastIndexOf("\\")||0)?"/":"\\")+1);
	g(id).value=path;
}
function createFlashUploader(opt,callbacks){
	...
	var option=extendProperty(callbacks,{createOptions:{id:"flash",url:opt.flashUrl,width:opt.width,height:opt.height,errorMessage:lang.flashError,wmode:browser.safari?"transparent":"window",ver:"10.0.0",vars:opt,container:opt.container}});
...
function createCopyButton(id,dataFrom){
	...
	var clipinterval=null;
	clipinterval=window.setInterval(function(){
		if(clipboard&&clipboard.flashInit){
			window.clearInterval(clipinterval);
