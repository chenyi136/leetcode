(function(){
	var remoteImage=null,uploadImage=null,onlineImage=null,searchImage=null;
	...
	function initTabs(){
		...
		setTabFocus(img&&img.tagName&&img.tagName.toLowerCase()=="img"?"remote":"upload");
	}
	function setTabFocus(id){
		...
		switch(id){
			case "remote":remoteImage=remoteImage||new RemoteImage();break;
			case "upload":
	...
	function initButtons(){
		dialog.onok=function(){
			var remote=false,list=[],id=null,tabs=$G("tabhead").children;
			for(var i=0;i<tabs.length;i++){
			...
			switch(id){
				case "remote":list=remoteImage.getInsertList();break;
				case "upload":
					...
					break;
				case "online":list=onlineImage.getInsertList();break;
				case "search":
	...
	RemoteImage.prototype={
		...
		initContainer:function(){
			this.dom={"url":$G("url"),"width":$G("width"),"height":$G("height"),"border":$G("border"),"vhSpace":$G("vhSpace"),"title":$G("title"),"align":$G("align")};
			var img=editor.selection.getRange().getClosedNode();
			if(img){
				this.setImage(img);
			}
		...
		initEvents:function(){
			...
			domUtils.on($G("width"),"keyup",function(){
				updatePreview();
				if(locker.checked){
					$G("height").value=Math.round(this.value/locker.getAttribute("data-proportion"));
				}else{
					_this.updateLocker();
				}
			});
			domUtils.on($G("height"),"keyup",function(){
				updatePreview();
				if(locker.checked){
					$G("width").value=Math.round(this.value*locker.getAttribute("data-proportion"));
				}else{
					_this.updateLocker();
				}
			});
		...
		setImage:function(img){
			...
			var wordImgFlag=img.getAttribute("word_img"),src=wordImgFlag?wordImgFlag.replace("&amp;","&"):(img.getAttribute("_src")||img.getAttribute("src",2).replace("&amp;","&")),align=editor.queryCommandValue("imageFloat");
			if(src!==$G("url").value){
				$G("url").value=src;
			}
			if(src){
		...
		setPreview:function(){
			var url=utils.unhtmlForUrl($G("url").value);
			if(url){
				var title=utils.unhtml($G("title").value);
				...
				var width=(!ow||!oh)?preview.offsetWidth:Math.min(ow,preview.offsetWidth);
				width=width+border*2>preview.offsetWidth?width:(preview.offsetWidth-border*2);
				var height=(!ow||!oh)?"":width*oh/ow;
		...
		getInsertList:function(){
			...
			if(data["url"]){
				return [{src:data["url"],_src:data["url"],width:data["width"]||"",height:data["height"]||"",border:data["border"]||"",floatStyle:data["align"]||"",vspace:data["vhSpace"]||"",title:data["title"]||"",alt:data["title"]||"",style:"width:"+data["width"]+"px;height:"+data["height"]+"px;"}];
			}else{
				return [];
			}
		}
	...
	UploadImage.prototype={
		...
		initUploader:function(){
			...
			uploader=_this.uploader=WebUploader.create({
				...
				compress:editor.getOpt("imageCompressEnable")?{width:imageCompressBorder,height:imageCompressBorder,quality:90,allowMagnify:false,crop:false,preserveHeaders:true}:false
			});
		...
		getInsertList:function(){
			var list=[],align=getAlign(),prefix=editor.getOpt("imageUrlPrefix");
			for(var i=0;i<this.imageList.length;i++){
				var data=this.imageList[i];
				list.push({src:prefix+data.url,_src:prefix+data.url,title:data.title,alt:data.original,floatStyle:align});
			}
			return list;
		}
	};
		...
		getInsertList:function(){
			...
			for(var i=0;i<lis.length;i++){
				if(domUtils.hasClass(lis[i],"selected")){
					...
					list.push({src:src,_src:src,alt:src.substr(src.lastIndexOf("/")+1),floatStyle:align});
				}
			}
			return list;
		}
	...
	SearchImage.prototype={
		...
		getImageData:function(){
			var _this=this,key=$G("searchTxt").value,type=$G("searchType").value,keepOriginName=editor.options.keepOriginName?"1":"0",url="http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word="+_this.encodeToGb2312(key)+type+"&keeporiginname="+keepOriginName+"&"+new Date;
					...
					if(json&&json.data){
						for(var i=0;i<json.data.length;i++){
							if(json.data[i].objURL){
								list.push({title:json.data[i].fromPageTitleEnc,src:json.data[i].objURL,url:json.data[i].fromURL});
							}
						}
					}
					_this.setList(list);
		...
		setList:function(list){
			var _this=this,listUl=$G("searchListUl");
			listUl.innerHTML="";
			if(list.length){
				for(var i=0;i<list.length;i++){
					var item=document.createElement("li");
		...
		getInsertList:function(){
			var align=getAlign(),list=[],items=$G("searchListUl").children;
			for(var i=0;i<items.length;i++){
				var child=items[i].firstChild&&items[i].firstChild.firstChild;
