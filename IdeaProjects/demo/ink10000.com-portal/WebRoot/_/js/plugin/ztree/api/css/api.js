var apiContent={
	...
	updateNodes:function(highlight){
		...
		function_zTree=$.fn.zTree.getZTreeObj("functionTree"),
		node=null;
		for(var i=0;i<apiContent.searchNodes.length;i++){
	...
	overlayAutoClose:function(e){
		...
		if(eId==="overlayDiv"||eId==="overlayDivArrow"||eClass.indexOf("searchPrev")>-1||eClass.indexOf("searchNext")>-1||eRel){
			return;
		}
	...
	overlayAjax:function(treeId,node){
		...
		$.ajax({
			...
			error:function(XMLHttpRequest,textStatus,errorThrown){
				window.alert("如果你使用的是Chrome浏览器，那么请把API文档发布到web服务目录下访问。\n\n（Chrome浏览器不支持本地ajax访问,即：file://）");
				if(apiContent.tmpDiv){
					apiContent.tmpDiv.empty();
				}
				node.isAjax=false;
